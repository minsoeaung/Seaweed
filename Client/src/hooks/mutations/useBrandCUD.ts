    ...
    return useMutation(
        async (data: CreateType | UpdateType | DeleteType) => {
            ...
            if (type === 'CREATE') {
                ...
                formData.set('name', data.brand.name);
                if (data.brand.files?.length) formData.set('picture', data.brand.files[0]);
        ...
        },
        {
            onSuccess: async (_, data) => {
                ...
                if (data.type === 'DELETE') queryClient.removeQueries([BRANDS, String(data.id)]);

                if (data.type === 'UPDATE') await queryClient.invalidateQueries([BRANDS, String(data.id)]);
            },
        }
    );
};
