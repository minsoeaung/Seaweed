...
const fetchProducts = async ({ queryKey }: QueryFunctionContext<[string, string]>): Promise<PagedResponse<Product>> => {
    ...
    return await ApiClient.get<never, PagedResponse<Product>>(
        `api/Products?pageNumber=${searchParams.get('pageNumber') || 1}&pageSize=${
            searchParams.get('pageSize') || 25
        }&orderBy=${searchParams.get('orderBy') || ''}&brands=${searchParams.get('brands') || ''}&categories=${
...
export const usePaginatedProducts = (searchParams: string) => {
    return {
        ...useQuery([PRODUCTS, searchParams], fetchProducts, {
